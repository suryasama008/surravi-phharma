...
export async function generateMetadata({ params }) {
  ...
  return {
    title: cc?.metaTitle || `${cfg.label} Supplier Hyderabad`,
    description: cc?.metaDescription || `Buy pharma-grade ${cfg.label} in Hyderabad. IP/BP/EP/USP grades. Ready stock. COA & MSDS. Pan India supply. Call ${SITE.phone}.`,
  };
}
  ...
  return (
    <div className="page-wrap">
      ...
      <div className="container">
        ...
        {/* ── Category header ── */}
        <h1 className="page-title">{cc?.h1 || cfg.label}</h1>
        <p className="page-sub">
          {products.length} products listed · IP/BP/EP/USP · Ready Stock · Pan India Supply
        </p>

        {/* ── Keyword-rich intro (only when content exists) ── */}
        {cc && (
